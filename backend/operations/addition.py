...
async def add_matrices(data: TwoMatrixInput):
    ...
    if error_validacion_dim: return ApiResponse(success=False, error=error_validacion_dim)
    ...
    except ValueError as e: return ApiResponse(success=False, error=str(e))
    
    resultado_frac = [[Fraction(0) for _ in range(num_columnas)] for _ in range(num_filas)]
    pasos = ["Inicio de la suma de matrices A y B."]
    ...
    if num_columnas > 0: 
        for i in range(num_filas):
            for j in range(num_columnas):
                ...
                suma_elemento_frac = a_ij + b_ij
                resultado_frac[i][j] = suma_elemento_frac
                str_a_ij = format_fraction_output(a_ij)
                str_b_ij = format_fraction_output(b_ij)
                str_suma = format_fraction_output(suma_elemento_frac)
                pasos.append(f"  C({i+1},{j+1}) = A({i+1},{j+1}) + B({i+1},{j+1}) = {str_a_ij} + {str_b_ij} = {str_suma}")
                
    resultado_output = [[format_fraction_output(el) for el in row] for row in resultado_frac]
    pasos.append("Suma completada.")
